...
loadImage = (file) => {
    ...
    img.onload = () => {
      ...
      // Downsample the image before processing
      const downscaledCanvas = document.createElement('canvas');
      const downscaledContext = downscaledCanvas.getContext('2d');
      downscaledCanvas.width = img.width / 2; // Adjust the downsampling factor as needed
      downscaledCanvas.height = img.height / 2;
      downscaledContext.drawImage(img, 0, 0, downscaledCanvas.width, downscaledCanvas.height);

      const downscaledImageData = downscaledContext.getImageData(0, 0, downscaledCanvas.width, downscaledCanvas.height);
      
      window.URL.revokeObjectURL(this.src);
      worker.addEventListener("message", handleWorkerCompletion, false);
      worker.postMessage({
          "imageData": downscaledImageData.data
      });
